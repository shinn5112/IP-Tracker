...
if [[ $answer == "y" ]]
then echo "0 0 * * * /opt/wan/auto-update.sh 2>> /opt/wan/update_errorlog.txt" | sudo crontab -u root -; sudo chown root:root update_errorlog.txt; sudo chmod 740 update_errorlog.txt; echo "Auto updates enabled."
fi
