...
if [[ $answer == "y" ]]
then echo "Installing auto-update crontab."; (sudo crontab -u root -l; echo -n "* * * * * "; echo -n $localRepo " 2>> /opt/wan/updateErrorLog.txt") | sudo crontab -u root -; sudo touch updateErrorLog.txt updateLog.txt; sudo chown root:root updateErrorLog.txt updateLog.txt; sudo chmod 740 updateErrorLog.txt updateLog.txt; sudo chmod -x updateErrorLog.txt updateLog.txt; echo "Auto updates enabled."
fi
